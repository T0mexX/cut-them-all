...
class GameBase   extends PApplet {
  ...
  class UpdateTimer(val fps: Float, ups: Float) {

    private val frameDuration: Float = 1000 / fps
    private val updateInterval: Float = 1000 / ups
    private var nextFrame: Float = Float.MaxValue
    private var nextUpdate: Float = Float.MaxValue
    private var updatesCount: Int = 0
    private var nextFpsDisplay: Float = Float.MaxValue


    def init(): Unit = {
      nextUpdate = currentTime() + updateInterval
      nextFrame = currentTime() + frameDuration
      nextFpsDisplay = currentTime() + 1000f
    }
    def timeForNextFrame: Boolean = currentTime() >= nextFrame
    def advanceFrame(): Unit = nextFrame += frameDuration
    def timeForNextUpdate: Boolean = currentTime() >= nextUpdate
    def advanceUpdate(): Unit = {
      updatesCount += 1
      if (currentTime() >= nextFpsDisplay) {
        println(updatesCount)
        updatesCount = 0
        nextFpsDisplay = currentTime() + 1000f
      }
